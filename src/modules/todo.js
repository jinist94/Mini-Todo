...
const todoReducer = (state = initialTodos, action) => {
  ...
  switch (action.type) {
    ...
    case DELETE_STEP:
      return produce(state, (draft) => {
        const todoIndex = draft.todos.findIndex((todo) => todo.id === data.todoId);
        const newSteps = draft.todos[todoIndex].steps.filter((step) => step.id !== data.stepId);
        draft.selectedTodo.todoData.steps = newSteps;
      });
    case UPDATE_STEP_CHECK:
      return produce(state, (draft) => {
        const todoIndex = draft.todos.findIndex((todo) => todo.id === data.todoId);
        const isCheck = draft.todos[todoIndex].steps[data.index].check ? false : true;
